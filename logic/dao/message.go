...
func (*messageDao) Add(ctx *context.Context, message model.Message) error {
	_, err := ctx.Session.Exec("insert into t_message(user_id,sender_type,sender_id,sender_device_id,receiver_type,receiver_id,type,content,sequence) values(?,?,?,?,?,?,?,?,?)",
...
func (*messageDao) ListByUserIdAndSequence(ctx *context.Context, userId int64, sequence int64) ([]*model.Message, error) {
